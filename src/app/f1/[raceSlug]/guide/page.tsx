...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { ...BASE_OG,title: `${race.city} F1 2026 Travel Guide: Everything You Need`,
      description: `Travel guide for the ${race.name}. Local tips, getting there, where to stay, and what to do in ${race.city}.`,
...
export default async function F1GuidePage({ params }: Props) {
  const { raceSlug } = await params;
  const race = await getRaceBySlug(raceSlug, 'f1');
  if (!race) notFound();
  ...
  return (
    ...
    <div className="min-h-screen pt-24 pb-24 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <RaceSubNav raceSlug={raceSlug} series="f1" current="guide" />
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {guideSections.map((sec, i) => (
                ...
                >
                  ...
                  >
                    <span className="text-xl">{getSectionIcon(sec.heading)}</span>
          ...
          <section>
            ...
            <GuideNavCards subPages={SUB_PAGES} raceSlug={raceSlug} accentColor={theme.accent} series="f1" />
          </section>
