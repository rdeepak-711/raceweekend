...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${race.city} MotoGP 2026 Travel Guide — Best Experiences, Hotels & Tips`;
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <section className="mb-16">
            ...
            <h1 className="font-display font-black text-4xl md:text-6xl text-white uppercase italic tracking-tighter leading-none mb-6">
              {race.city} <span className="text-[var(--text-tertiary)]">MotoGP 2026</span>
            </h1>
          ...
          {featured.length > 0 && (
            <section className="mb-16">
              <div className="flex items-center justify-between mb-8">
                ...
                <Link href={`/motogp/${raceSlug}/experiences`} className="text-sm font-bold text-[var(--accent-teal)] hover:underline">
                  View all activities →
                </Link>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {featured.map((exp, i) => (
                  <ExperienceCard 
                    ...
                    experience={exp} 
                    index={i} 
                    detailHref={`/motogp/${raceSlug}/experiences/${exp.slug}`}
                  />
                ))}
              </div>
            </section>
