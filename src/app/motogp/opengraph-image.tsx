...
export const alt = 'MotoGP 2026 Grand Prix Calendar & Travel Guides | Race Weekend';
  ...
  return new ImageResponse(
    (
      <div style={{ display: 'flex', width: 1200, height: 630, background: '#15151E', position: 'relative', overflow: 'hidden' }}>
        {bgData && (
          <img src={bgData} style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', opacity: 0.4 }} />
        )}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(255,107,0,0.55) 0%, rgba(21,21,30,0.75) 50%, rgba(21,21,30,0.9) 100%)' }} />
        ...
        <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '72px 88px', width: '100%' }}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
            <div style={{ background: '#FF6B00', borderRadius: 8, padding: '5px 16px', color: 'white', fontSize: 14, fontWeight: 800, letterSpacing: 1 }}>
              🏍 MotoGP
            </div>
          ...
          <div style={{ fontSize: 26, color: 'rgba(255,255,255,0.6)', marginTop: 14 }}>
            Session schedules · Curated experiences · Tickets · Travel guides
          </div>
